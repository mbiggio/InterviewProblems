...
/*
Find the largest subset S of a set of number such that (si % sj == 0) or (sj % si == 0)
for each si,sj in S.
*/
...
struct item {
   int previous;
   int size;
   item(): previous(0), size(0) {};
   item(int p, int s): previous(p), size(s) {}
};
...
public:
   vector<int> largestDivisibleSubset(vector<int>& nums) {
      ...
      for (int k=0; k<nums.size(); k++) {
         ...
         int max_idx = 0;         
         for (int j=0; j<k; j++) {
            if (nums[k]%nums[j]==0 && indexes[j].size > max_size) {
               max_size = indexes[j].size;
               max_idx = j;
            }
         }
         if (max_size == 0) {
            ...
            indexes[k] = item(max_idx,max_size+1);
         }                        
      }
